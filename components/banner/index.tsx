...
// Define the type for the media item
interface MediaItem {
  id: number;
...
export default function Banner({ medias }: BannerProps) {
  ...
  return (
    <div className="flex flex-col space-y-2 py-16 md:space-y-4 lg:h-[65vh] lg:justify-end lg:pb-12 lg:pl-24">
      <div className="absolute top-0 left-0 h-[95vh] w-screen -z-10">
        <Image
          src={`${baseUrl}/${
            createRandomMedia?.backdrop_path ||
            createRandomMedia?.poster_path ||
            ""
          }`}
          alt="Banner"
          layout="fill"
      ...
      <p className="max-w-xs text-shadow-md text-xs md:max-w-lg md:text-lg lg:max-w-2xl line-clamp-5">
        {createRandomMedia?.overview || "No overview available."}
      </p>
